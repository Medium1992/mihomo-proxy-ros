...
# Запасной путь на случай, когда панель открыли не кнопкой, а прямым
# заходом на /ui/. Кнопка передаёт адрес и секрет во фрагменте ссылки, и
# Nuxt-сборка metacubexd их принимает — проверено на её текущей сборке.
# Но при прямом заходе параметров нет, и в форме остаётся её собственный
# http://127.0.0.1:9090. Её index.html подключает config.js с ключом
# defaultBackendURL, чем мы и пользуемся: панель отдаёт сам контроллер,
# поэтому location.origin в браузере всегда указывает ровно на тот адрес и
# порт, по которым человек её открыл, каким бы из адресов контейнера он ни
# пользовался. Секрет так передать нельзя — его несёт только ссылка.
...
# BASIC_AUTH_HASH — ГОТОВЫЙ md5-хеш ($1$...); генерится на странице
# «Инструменты». Плейн-пароль тоже работает (busybox сравнит как есть),
# но хранить его в env не стоит. Дефолт: admin / admin.
setup_basic_auth() {
...
# Поэтому поднимаем второй httpd, который:
#   * слушает ТОЛЬКО 127.0.0.1 → из LAN недоступен;
#   * раздаёт вебрут с единственным CGI — конвертером подписок.
# Ничего читающего/пишущего файлы (read-file, save-file, draft) тут нет.
# Отключить: WEB_API_PORT=0
setup_api_listener() {
