...
EOF
  ...
  <small>Кнопка <b>Панель Mihomo</b> в шапке открывает контроллер на порту <code>9090</code>. Zashboard и Yacd-meta принимают адрес и пароль прямо в ссылке и подключаются сами. Свежие сборки MetaCubeXD параметры из ссылки игнорируют, поэтому контейнер прописывает адрес контроллера в её <code>config.js</code>: форма подключения открывается уже заполненной, пароль из <code>UI_SECRET</code> вводится один раз и запоминается панелью.</small>
  <i>preset</i>
</label>
EOF
}
