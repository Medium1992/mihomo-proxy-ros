...
EOF
  ...
  <small>Кнопка <b>Панель Mihomo</b> в шапке открывает контроллер на порту <code>9090</code> и подставляет в ссылку адрес контейнера с паролем из <code>UI_SECRET</code>, поэтому панель подключается сама. Способ передачи у каждой панели свой: MetaCubeXD читает параметры из фрагмента ссылки, Zashboard и Yacd-meta — из query string.</small>
  <i>preset</i>
</label>
EOF
}
